 ...
 * if greater is 1, greater comparison, if 0, less */ 
int compare_brightness(const void* a, const void* b, int greater) {
  pixel* p_a = (pixel*)a;
  pixel* p_b = (pixel*)b;
  return ((p_a->r + p_a->g + p_a->b) (greater ? > : <) (p_b->r + p_b->g + p_b->b));
}
